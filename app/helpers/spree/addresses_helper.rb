      ...
      content_tag :div, id: [address_id, method].join, class: 'form-group' do
        if handler
          ...
          form.label(method) + separator.html_safe +
            form.text_field(method, class: [is_required ? 'required' : nil, 'form-control input-text'].compact, required: is_required)
        end
      end
    end
    ...
    def address_state(form, country, _address_id = 'b')
      ...
      state_elements = [
        form.collection_select(:state_id, country.states.order(:name),
                              :id, :name,
                              { include_blank: true },
                               class: have_states ? 'form-control chosen-select' : 'hidden',
                               disabled: !have_states) +
          form.text_field(:state_name,
                          class: !have_states ? 'form-control input-text' : 'hidden',
                          disabled: have_states)
      ].join.tr('"', "'").delete("\n")

      form.label(:state, Spree.t(:state)) + '<span class="req">*</span><br />'.html_safe +
        content_tag(:noscript, form.text_field(:state_name, class: 'required form-control input-text')) +
