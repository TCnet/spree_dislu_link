module Spree
  module BaseHelper
    ...
    # add *_image_url methods to return image url 
    def method_missing(method_name, *args, &block)
      if image_style = image_url_style_from_method_name(method_name)
        define_image_url_method(image_style)
        send(method_name, *args)
      elsif image_style = image_style_from_method_name(method_name)
        define_image_method(image_style)
        send(method_name, *args)
      else
        super
    ...
    # image_url meghtods
    def create_product_image_url(image, product, style)
    ...
    def define_image_url_method(style)
      self.class.send :define_method, "#{style}_image_url" do |product|
        if product.images.empty?
          if !product.is_a?(Spree::Variant) && !product.variant_images.empty?
            create_product_image_url(product.variant_images.first, product, style)
          elsif product.is_a?(Spree::Variant) && !product.product.variant_images.empty?
            create_product_image_url(product.product.variant_images.first, product, style)
          else
             asset_path("noimage/#{style}.png")
          end
        else

          create_product_image_url(product.images.first, product, style)
